...
/**
 * InvestorLens - Master Test Runner CLI (.cjs entrypoint)
 * Discovers and runs test suites across Tier 1, Tier 2, Tier 3, Tier 4.
 *
 * Usage:
 *   node tests/run-all-tests.cjs
 *   node tests/run-all-tests.cjs --tier=tier1
 *   node tests/run-all-tests.cjs --tier=tier1,tier2
 *   node tests/run-all-tests.cjs --filter=feature-01
 *   node tests/run-all-tests.cjs --verbose
 */
